// frontend/src/utils/constants.ts
export const NODE_URL = "https://fullnode.devnet.aptoslabs.com"; // Replace with your Aptos node URL
export const FAUCET_URL = "https://faucet.devnet.aptoslabs.com"; // Replace with your Aptos faucet URL

// Update with your actual smart contract address once deployed
export const MODULE_ADDRESS = "0x1";  // Replace with your module address on Aptos

// Default royalty percentage for new NFTs
export const DEFAULT_ROYALTY_PERCENTAGE = 10; 

// Maximum allowed royalty percentage
export const MAX_ROYALTY_PERCENTAGE = 50; 

// Default image dimensions for the NFT creation form
export const DEFAULT_IMAGE_WIDTH = 512; 
export const DEFAULT_IMAGE_HEIGHT = 512;

// API endpoint for AI art generation
export const AI_GENERATION_API = "https://api-inference.huggingface.co/models/CompVis/stable-diffusion-v1-5";
// add API_URL to constants.ts
export const API_URL = "https://api-inference.huggingface.co/models/CompVis/stable-diffusion-v1-5";

// ... Add more constants as needed ...
